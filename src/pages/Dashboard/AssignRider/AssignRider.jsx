...
const AssignRider = () => {
  ...
  const confirmAssignRider = async () => {
    ...
    try {
      ...
      alert(`Rider ${selectedRider.name} assigned to parcel ${selectedParcel.tracking_id}`);
      closeModal();
    } catch (error) {
      alert('Failed to assign rider',error);
    }
  };
                  ...
                  {riders.map((rider) => (
                    <li
                      ...
                      className={`p-2 cursor-pointer rounded mb-2 border ${
                        selectedRider?._id === rider._id ? 'bg-lime-200 border-lime-500' : 'border-gray-300'
                      }`}
                    >
